"""
Collect CARLA images for TRAINING.
Uses SAME weather conditions as evaluation but collects DIFFERENT images.
Saves data in HDF5 format for efficient PyTorch loading.

IMPORTANT: This creates a training dataset. Use collect_eval_data.py to create
a separate evaluation dataset with distinct images from the same weather conditions.
"""
...
class TrainingDataCollector:
    """
    Collects training data from CARLA.
    Uses SAME weather scenarios as evaluation but DIFFERENT images.
    """
    
    # # Evaluation scenarios to AVOID (prevent data leakage)
    # EVAL_SCENARIOS = {
    #     'clear_noon', 'cloudy', 'light_rain', 'heavy_rain', 'fog',
    #     'clear_sunset', 'cloudy_sunset', 'wet_cloudy_sunset'
    # }
    
    # # Training weather presets (diverse but different from eval)
    # # Balanced set: 8 conditions for ~40GB with 8000 frames each
    # TRAINING_WEATHERS = [
    #     'clear_dawn',           # Early morning, clear
    #     'cloudy_dawn',          # Cloudy morning
    #     'wet_noon',             # Wet midday
    #     'mid_rainy_noon',       # Medium rain
    #     'hard_rain_noon',       # Heavy rain
    #     'soft_rain_sunset',     # Light rain evening/sunset
    #     'clear_night',          # Night driving, clear
    #     'wet_cloudy_night',     # Wet night (wet+cloudy+night)
    # ]

    # Training weather scenarios - SAME as evaluation scenarios
    # This ensures same weather conditions, but images will be distinct
    # from evaluation data due to different spawn points/trajectories
    TRAINING_WEATHERS = [
    ...
    def __init__(self, host='localhost', port=2000, output_dir='./training_data', random_seed=12345):
        """
        ...
        Args:
            ...
            random_seed: Random seed for spawn point selection (different from eval)
        """
    ...
    def collect_dataset(self, frames_per_weather=700, num_vehicles=5, resume=True):
        """
        Collect comprehensive training dataset.

        Args:
            frames_per_weather: Number of frames per weather condition
        ...
        # Create HDF5 file
        h5_path = self.output_dir / 'training_data.h5'
        ...
        # Check disk space
        self.check_disk_space(required_gb=50)

        with h5py.File(h5_path, mode) as h5f:
            # Create or get existing datasets
            max_samples = len(self.TRAINING_WEATHERS) * frames_per_weather * num_vehicles
            ...
            # Update metadata
            h5f.attrs['weathers'] = self.TRAINING_WEATHERS
            h5f.attrs['frames_per_weather'] = frames_per_weather
            h5f.attrs['num_vehicles'] = num_vehicles
            
            # Collect from each weather condition
            for weather_idx, weather in enumerate(self.TRAINING_WEATHERS):
                # Skip if already completed
                if weather in completed_weathers:
                    print(f"\n{'='*70}")
                    print(f"Weather {weather_idx+1}/{len(self.TRAINING_WEATHERS)}: {weather} [SKIPPING - already completed]")
                    print(f"{'='*70}")
                    continue

                print(f"\n{'='*70}")
                print(f"Weather {weather_idx+1}/{len(self.TRAINING_WEATHERS)}: {weather}")
                print(f"{'='*70}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frames-per-weather', type=int, default=1000,
                       help='Frames to collect per weather condition')
    parser.add_argument('--num-vehicles', type=int, default=3,
                       help='Number of vehicle spawns per weather')
    parser.add_argument('--random-seed', type=int, default=12345,
                       help='Random seed for spawn point selection (use different from eval)')
